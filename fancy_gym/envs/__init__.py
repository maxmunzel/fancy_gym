...
for alpha in range(5, 30):
    for w_scale in [1, 3, 10]:
        for g_scale in [1, 3, 10]:
            for replan in [25, 50]:
                register(
                    id=f"Sweep107-alpha{alpha}-tau5-ws{w_scale}-gs{g_scale}-replan-{replan}",
                    entry_point="fancy_gym.utils.make_env_helpers:make_bb_env_helper",
                    kwargs={
                        "name": "BoxPushingTemporalSparse-v0",
                        "wrappers": [mujoco.box_pushing.MPWrapper],
                        "trajectory_generator_kwargs": {
                            "trajectory_generator_type": "prodmp",
                            "duration": 8.0,  # changed here from 102
                            "action_dim": 2,
